...
<input type="model" id="model" name="model" required><br><br>

<label for="Licensep">License plate:</label>
<input type="text" id="Licensep" name="Licensep" required><br><br>

<label for="Vehidnumber">Vehicle identification number:</label>
<input type="number" id="Vehidnumber" name="Vehidnumber" required><br><br>
...
include('database_connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        // Insert section
        $gms = $connection->prepare("INSERT INTO vehicles(Vehicle_Id, Customer_Id, Make, Model, Licence_Plate, Vehicle_Identification_Number	) VALUES (?, ?, ?, ?, ?, ?)");

        $gms->bind_param("issss", $Vehid, $Custid, $Mak, $Mod, $Lplate, $Vehidnumber);
        ...
        $Mak = $_POST['Make'];
        $Mod = $_POST['Model'];
        $Lplate = $_POST['Licence_Plate'];
        $Vehidnumber = $_POST['Vehicle_Identification_Number'];

        if ($gms->execute()) {
            echo "New record has been added successfully.<br><br>
                 <a href='Vehicles.html'>Back to Form</a>";
        } else {
    ...
    <center><h2>Table of Vehicles</h2></center>
    <table>
        <tr>
            <th></th>
            <th>Vehicle_Id</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_Vehicles.php?Item_Id=$Itid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_Vehicles.php?Item_Id=$Itid'>Update</a></td> 
                </tr>";
            }
        } else {
